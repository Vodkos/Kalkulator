...
def get_self_consumption_fraction(pv_size, battery_capacity, annual_energy_consumption):
    consumption_levels = sorted(self_consumption_matrix[battery_capacity].keys())
    pv_levels = sorted(self_consumption_matrix[battery_capacity][consumption_levels[0]].keys())

    closest_consumption_level = min(consumption_levels, key=lambda x: abs(x - annual_energy_consumption))
    closest_pv_level = min(pv_levels, key=lambda x: abs(x - pv_size))

    return self_consumption_matrix[battery_capacity][closest_consumption_level][closest_pv_level]
...
def find_optimal_pv_size(annual_energy_consumption, battery_capacity, pv_sizes, purchase_price, sell_price):
    ...
    optimal_self_consumed_energy = 0
    optimal_grid_feed_energy = 0
    optimal_grid_energy_usage = 0
    ...
    for pv_size in pv_sizes:
        ...
        if savings > max_savings and savings <= annual_energy_consumption * purchase_price:
            max_savings = savings
...
annual_energy_consumption = st.number_input("Roczne zużycie energii (kWh)", value=4000, step=50
                                           )
...
if optimal_pv_size is not None:
    ...
    st.write(f"**Ilość energii pobranej z sieci:** {grid_energy_usage:.2f} kWh")
